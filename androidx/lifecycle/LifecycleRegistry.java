  ...
  extends Lifecycle
{
  ...
  public void addObserver(@NonNull LifecycleObserver paramLifecycleObserver)
  {
    ...
    Lifecycle.State localState = Lifecycle.State.DESTROYED;
    ...
    localObject = (LifecycleOwner)this.mLifecycleOwner.get();
    if (localObject == null) {
      return;
    }
    int i;
    ...
    {
      pushParentState(localObserverWithState.mState);
      localObserverWithState.dispatchEvent((LifecycleOwner)localObject, upEvent(localObserverWithState.mState));
      popParentState();
      localState = calculateTargetState(paramLifecycleObserver);
    }
    if (i == 0) {
...
/* Location:              C:\Users\User\Desktop\decompile\jdgui110\Tapojar.jar!\androidx\lifecycle\LifecycleRegistry.class
