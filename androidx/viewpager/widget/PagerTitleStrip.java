  ...
  extends ViewGroup
{
  ...
  void updateTextPositions(int paramInt, float paramFloat, boolean paramBoolean)
  {
    ...
    label372:
    TextView localTextView = this.mCurrText;
    localTextView.layout(i5, j, m, localTextView.getMeasuredHeight() + j);
    j = Math.min(i2, i5 - this.mScaledTextSpacing - i);
    localTextView = this.mPrevText;
    localTextView.layout(j, i4, i + j, localTextView.getMeasuredHeight() + i4);
    i4 = Math.max(n - i3 - k, m + this.mScaledTextSpacing);
    localTextView = this.mNextText;
    localTextView.layout(i4, paramInt, i4 + k, localTextView.getMeasuredHeight() + paramInt);
    ...
    implements ViewPager.OnPageChangeListener, ViewPager.OnAdapterChangeListener
  {
    ...
    public void onChanged()
    {
      ...
      float f = localPagerTitleStrip.mLastKnownPositionOffset;
      if (f < 0.0F) {
        f = 0.0F;
      }
      localPagerTitleStrip.updateTextPositions(localPagerTitleStrip.mPager.getCurrentItem(), f, true);
    }
...
/* Location:              C:\Users\User\Desktop\decompile\jdgui110\Tapojar.jar!\androidx\viewpager\widget\PagerTitleStrip.class
