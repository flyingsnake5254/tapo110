  ...
  extends GoogleApiAvailabilityLight
{
  ...
  private final void zaa(Context paramContext, int paramInt, String paramString, PendingIntent paramPendingIntent)
  {
    ...
    Object localObject2 = ConnectionErrorMessages.getErrorNotificationMessage(paramContext, paramInt);
    paramString = paramContext.getResources();
    NotificationManager localNotificationManager = (NotificationManager)paramContext.getSystemService("notification");
    localObject1 = new NotificationCompat.Builder(paramContext).setLocalOnly(true).setAutoCancel(true).setContentTitle((CharSequence)localObject1).setStyle(new NotificationCompat.BigTextStyle().bigText((CharSequence)localObject2));
    if (DeviceProperties.isWearable(paramContext))
    {
      ...
      if (DeviceProperties.isWearableWithoutPlayStore(paramContext)) {
        ((NotificationCompat.Builder)localObject1).addAction(R.drawable.common_full_open_on_phone, paramString.getString(R.string.common_open_on_phone), paramPendingIntent);
      } else {
    ...
    {
      ((NotificationCompat.Builder)localObject1).setSmallIcon(17301642).setTicker(paramString.getString(R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setContentIntent(paramPendingIntent).setContentText((CharSequence)localObject2);
    }
  ...
  public Task<Void> checkApiAvailability(GoogleApi<?> paramGoogleApi, GoogleApi<?>... paramVarArgs)
  {
    ...
    for (int j = 0; j < i; j++) {
      Preconditions.checkNotNull(paramVarArgs[j], "Requested API must not be null.");
    }
...
/* Location:              C:\Users\User\Desktop\decompile\jdgui110\Tapojar.jar!\com\google\android\gms\common\GoogleApiAvailability.class
