...
public class CrashlyticsReportDataCapture
{
  ...
  static final String GENERATOR = String.format(Locale.US, "Crashlytics Android SDK/%s", new Object[] { "18.1.0" });
  ...
  {
    return CrashlyticsReport.builder().setSdkVersion("18.1.0").setGmpAppId(this.appData.googleAppId).setInstallationUuid(this.idManager.getCrashlyticsInstallId()).setBuildVersion(this.appData.versionCode).setDisplayVersion(this.appData.versionName).setPlatform(4);
  }
  ...
  private CrashlyticsReport.Session.Event.Application.Execution.Exception populateExceptionData(TrimmedThrowableData paramTrimmedThrowableData, int paramInt1, int paramInt2, int paramInt3)
  {
    ...
    paramTrimmedThrowableData = CrashlyticsReport.Session.Event.Application.Execution.Exception.builder().setType(str1).setReason(str2).setFrames(ImmutableList.from(populateFramesList(arrayOfStackTraceElement, paramInt1))).setOverflowCount(i);
    if ((localTrimmedThrowableData != null) && (i == 0)) {
  ...
  private CrashlyticsReport.Session.Event.Application.Execution.Thread.Frame populateFrameData(StackTraceElement paramStackTraceElement, CrashlyticsReport.Session.Event.Application.Execution.Thread.Frame.Builder paramBuilder)
  {
    ...
    return paramBuilder.setPc(l2).setSymbol(str).setFile((String)localObject).setOffset(l3).build();
  }
  ...
  private ImmutableList<CrashlyticsReport.Session.Event.Application.Execution.Thread.Frame> populateFramesList(StackTraceElement[] paramArrayOfStackTraceElement, int paramInt)
  {
    ...
    for (int j = 0; j < i; j++) {
      localArrayList.add(populateFrameData(paramArrayOfStackTraceElement[j], CrashlyticsReport.Session.Event.Application.Execution.Thread.Frame.builder().setImportance(paramInt)));
    }
  ...
  {
    CrashlyticsReport.Session.Application.Builder localBuilder = CrashlyticsReport.Session.Application.builder().setIdentifier(this.idManager.getAppIdentifier()).setVersion(this.appData.versionCode).setDisplayVersion(this.appData.versionName).setInstallationUuid(this.idManager.getCrashlyticsInstallId());
    String str = this.appData.unityVersionProvider.getUnityVersion();
    if (str != null) {
      localBuilder.setDevelopmentPlatform("Unity").setDevelopmentPlatformVersion(str);
    }
  ...
  private CrashlyticsReport.Session.Device populateSessionDeviceData()
  {
    ...
    return CrashlyticsReport.Session.Device.builder().setArch(i).setModel(Build.MODEL).setCores(j).setRam(l1).setDiskSpace(l2 * l3).setSimulator(bool).setState(k).setManufacturer(str).setModelClass((String)localObject).build();
  }
  ...
  private ImmutableList<CrashlyticsReport.Session.Event.Application.Execution.Thread> populateThreadsList(TrimmedThrowableData paramTrimmedThrowableData, Thread paramThread, int paramInt, boolean paramBoolean)
  {
    ...
    if (paramBoolean)
    {
      ...
      while (localIterator.hasNext())
      {
        ...
        paramTrimmedThrowableData = (Thread)localEntry.getKey();
        if (!paramTrimmedThrowableData.equals(paramThread)) {
          localArrayList.add(populateThreadData(paramTrimmedThrowableData, this.stackTraceTrimmingStrategy.getTrimmedStackTrace((StackTraceElement[])localEntry.getValue())));
        }
      }
    }
    return ImmutableList.from(localArrayList);
...
/* Location:              C:\Users\User\Desktop\decompile\jdgui110\Tapojar.jar!\com\google\firebase\crashlytics\internal\common\CrashlyticsReportDataCapture.class
