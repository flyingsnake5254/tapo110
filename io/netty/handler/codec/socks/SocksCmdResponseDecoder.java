  ...
  extends ReplayingDecoder<State>
{
  ...
  protected void decode(ChannelHandlerContext paramChannelHandlerContext, ByteBuf paramByteBuf, List<Object> paramList)
    throws Exception
  {
    int i = 1.$SwitchMap$io$netty$handler$codec$socks$SocksCmdResponseDecoder$State[((State)state()).ordinal()];
    if (i != 1)
    {
      if (i != 2)
      {
    ...
    this.cmdStatus = SocksCmdStatus.valueOf(paramByteBuf.readByte());
    ...
    if (i != 1)
    {
      if (i != 2)
      {
        ...
        else
        {
          ...
          paramList.add(new SocksCmdResponse(this.cmdStatus, this.addressType, (String)localObject, i));
        }
      }
      else
      {
