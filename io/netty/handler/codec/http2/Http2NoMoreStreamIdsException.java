package io.netty.handler.codec.http2;

public class Http2NoMoreStreamIdsException
  extends Http2Exception
{
  private static final String ERROR_MESSAGE = "No more streams can be created on this connection";
  private static final long serialVersionUID = -7756236161274851110L;
  
  public Http2NoMoreStreamIdsException()
  {
    super(Http2Error.PROTOCOL_ERROR, "No more streams can be created on this connection", Http2Exception.ShutdownHint.GRACEFUL_SHUTDOWN);
  }
  
  public Http2NoMoreStreamIdsException(Throwable paramThrowable)
  {
    super(Http2Error.PROTOCOL_ERROR, "No more streams can be created on this connection", paramThrowable, Http2Exception.ShutdownHint.GRACEFUL_SHUTDOWN);
  }
}


/* Location:              C:\Users\User\Desktop\decompile\jdgui110\Tapojar.jar!\io\netty\handler\codec\http2\Http2NoMoreStreamIdsException.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */